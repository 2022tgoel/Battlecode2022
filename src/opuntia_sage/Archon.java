...
public class Archon extends Unit {
    ...
    private int desiredLabs = 1000;
    private int desiredBuilders = 1000;
    ...
    public void run() throws GameActionException {
        ...
        troopCounter = new int[] {
                ...
                radio.readCounter(RobotType.LABORATORY)
        };

        if (round_num == 2) {
            int leadEstimate = radio.getLeadEstimate();
            desiredNumMiners = determineMinerNum(leadEstimate);
            desiredLabs = determineLabNum(leadEstimate);
            radio.writeChannel(CHANNEL.DESIRED_LABS, desiredLabs);
            desiredBuilders = determineBuilderNum(desiredLabs);
        }
        ...
        switch (mode) {
            ...
            case DEFAULT:
                // if not enough builders and labs weren't fully built, build builder
                if(troopCounter[2] < desiredBuilders && troopCounter[5] < desiredLabs){
                    System.out.println("Desired Builders: " + desiredBuilders + " Desired Labs: " + desiredLabs);
                    build(new int[]{0, 0, 1});
                }
                attemptHeal();
                // every 5th round, alternate which archon spawns
    ...
    public int determineLabNum(int leadEstimate) throws GameActionException {
        int numLab = 1;
        if(leadEstimate > 2000){
            numLab += 3;
        } else if(leadEstimate > 1000){
    ...
    public int determineBuilderNum(int labNum){
        return 1;
    }
