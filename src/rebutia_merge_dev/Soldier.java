...
public class Soldier extends Unit {
    ...
    /**
     * getInitialExploratoryLocation() gets the location when you extend the vector
     * from your location
    ...
    public void huntTarget() throws GameActionException {
        ...
        if (rc.canSenseRobotAtLocation(target) && rc.senseRobotAtLocation(target).type == RobotType.ARCHON) {
            moveInDirection(lastAttackDir);
        } else if (rc.getLocation().distanceSquaredTo(target) <= 13) {
            // check for low rubble squares to move to
            Direction lowRubble = findLowRubble();
            if (lowRubble != null && rc.canMove(lowRubble)) {
                rc.move(lowRubble);
            }
        } else {
            moveInDirection(lastAttackDir);
    ...
    public void fleeLowRubble(int[] dir) throws GameActionException {
        ...
        Direction[] sorted_dirs = { d, d.rotateLeft(), d.rotateRight(), d.rotateLeft().rotateLeft(),
                d.rotateRight().rotateRight(), d.opposite().rotateRight(), d.opposite().rotateLeft(), d.opposite() };
        int a = 6;
        int lowestCost = 100000;
        Direction bestDir = null;
        for (int i = 0; i < 8; i++) {
            if (!rc.canMove(sorted_dirs[i]))
                continue;
