...
public class Sage extends Unit {
     ...
     * // if robot should be able to see archon but can't, inform everyone that
     * archon is dead
     ...
     * 
     * // increment repulsion
     * if ((Math.abs(robot.location.x - loc.x) + Math.abs(robot.location.y - loc.y))
     * <= 4) {
     ...
     * }
     * }
     * rc.setIndicatorString("dir: " + d + "| attraction: " + Math.round(dx1) + ", "
     * + Math.round(dy1) + " | repulsion: " + Math.round(dx2) + ", " +
     * Math.round(dy2));
     * // rc.setIndicatorString("vs: " + Math.round(cxs) + ", " + Math.round(cys) +
     * " | vm: " + Math.round(cxm) + ", " + Math.round(cym));
     * return d;
     * }
     * 
     ...
     * MapLocation center = new MapLocation(rc.getMapWidth()/2,
     * rc.getMapHeight()/2);
     * if (center.x - cur.x > 0) {
     * if (center.y - cur.y > 0) {
     * d = Direction.NORTHEAST;
     ...
     * rc.senseNearbyRobots(RobotType.SOLDIER.actionRadiusSquared,
     * rc.getTeam().opponent());
