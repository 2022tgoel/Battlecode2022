...
public class Miner extends Unit {
    ...
    public MODE getMode(int amountMined) throws GameActionException {
        ...
        if (target != null) {
            if (rc.canSenseLocation(target) && getValue(target) <= 1) {
                ...
                return MODE.MINE_DISCOVERED; // continue going to it
        }
    ...
    /**
     * findMiningArea()
     * Returns the location of the most lucrative mining area outside of mining
     * radius
     **/
    public MapLocation findMiningArea() throws GameActionException {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            if (data != 0) {
                ...
                int y = data % 64;
                MapLocation dest = new MapLocation(x, y);
                if (rc.getLocation().distanceSquaredTo(dest) < 200)
                    return dest; // within range
            }
        }
