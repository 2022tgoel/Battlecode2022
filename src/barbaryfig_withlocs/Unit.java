...
public class Unit {
    ...
    public boolean adjacentToEdge() throws GameActionException {
        ...
        int height = rc.getMapHeight();
        int width = rc.getMapWidth();
        return (height - y < 3 || y < 3 || x < 3 || width - x < 3);
    }
    ...
    public void tallyUnitTotals() throws GameActionException {
        int channel = -1;
        switch (rc.getType()) {
            case MINER:
                channel = CHANNEL.MINERS_ALIVE.getValue();
                break;
            case SOLDIER:
                channel = CHANNEL.SOLDIERS_ALIVE.getValue();
                break;
            case BUILDER:
                channel = CHANNEL.BUILDERS_ALIVE.getValue();
                break;
            default:
                break;
        }
