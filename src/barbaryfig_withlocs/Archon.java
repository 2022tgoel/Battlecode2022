...
public class Archon extends Unit {
    ...
    public void run() throws GameActionException {
        ...
        radio.updateRoundNumber();
        radio.clearThreat();
        radio.clearMiningAreas();

        if (Comms.isTallyingRound(roundNum)) {
            radio.clearGridSquares();
        } else if (Comms.isTallyingRound(roundNum - 1)) {
            // If we just completed a tallying, round, we need to update the
            // list of the most active grid squares.
            int topN = 4;
            int mostActiveGridSquares[] = new int[topN];
            int mostActiveGridSquareCounts[] = new int[topN];

            for (int i = topN; --i >= 0;) {
                mostActiveGridSquareCounts[i] = -1;
            }

            int gridSquare = CHANNEL.GRID_SIZE * CHANNEL.GRID_SIZE;
            while (--gridSquare >= 0) {
                int gridSquareValue = Comms.getGridSquareValue(rc, CHANNEL.GRID_BASE.getValue(), gridSquare,
                        CHANNEL.GRID_SQUARES_PER_CHANNEL);

                // Bubble up. mostActiveGridSquareCounts[topN - 1] is the most active
                for (int i = 0; i < topN; i++) {
                    if (gridSquareValue > mostActiveGridSquareCounts[i]) {
                        if (i > 0) {
                            // Shift the existing square down an index
                            int oldSquare = mostActiveGridSquares[i];
                            int oldCount = mostActiveGridSquareCounts[i];
                            mostActiveGridSquares[i - 1] = oldSquare;
                            mostActiveGridSquareCounts[i - 1] = oldCount;
                        }
                        mostActiveGridSquares[i] = gridSquare;
                        mostActiveGridSquareCounts[i] = gridSquareValue;
                    } else {
                        break;
                    }
                }
        ...
        if (roundNum % num_archons_alive == archonNumber) {
            radio.clearCounts();
        }
        ...
        switch (mode) {
            case THREATENED:
                ...
                if (roundNum % tot != threatChannel) { // alternate between those under threat
                    return;
                }
