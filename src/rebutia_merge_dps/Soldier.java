...
public class Soldier extends Unit {
    ...
    public int getAttackPriority(RobotInfo bot, boolean attackSupport) {
        double healthRatio = bot.getHealth()/((double)bot.type.health);
        double typePriority = getAttackPriority(bot.type, attackSupport);
        int cooldownMult = 1;
        try {
            cooldownMult += rc.senseRubble(bot.location)/10;
        } catch (GameActionException ignored){}

        double tot = 0;
        tot += 1000.*typePriority;
        tot += 10.*(1.-healthRatio);
        tot += 100. * (11-cooldownMult); // favor attacking lower rubble / higher dps people
        return (int)tot;
    }

    // returns priority of attacking this type [0, 1]
    public double getAttackPriority(RobotType type, boolean attackSupport){
        // sage, soldier, tower, miner, builder, archon
        switch (type){
            case SAGE:
                return 1;
            case SOLDIER:
            ...
            case LABORATORY:
                return 0.4;
            case MINER:
                return attackSupport ? 0.3 : -1;
            case BUILDER:
                return attackSupport ? 0.2 : -1;
            case ARCHON:
                return 0.1;
            default:
                return 0;
    ...
    public boolean attemptAttack(boolean attackMiners) throws GameActionException {
        RobotInfo newTarget = attemptAttackNew(attackMiners);

        return newTarget != null;
    }

    public RobotInfo attemptAttackNew(boolean attackSupport) throws GameActionException {
        RobotInfo[] nearbyBots = rc.senseNearbyRobots(RobotType.SOLDIER.actionRadiusSquared, rc.getTeam().opponent());
        Arrays.sort(nearbyBots, Comparator.comparingInt(b -> {
            return -getAttackPriority(b, attackSupport);

        }));

        // if there are any nearby enemy robots, attack the one with the least health
        for(int i = 0; i < nearbyBots.length; i++){
            RobotInfo targetBot = nearbyBots[i];
            if(!attackSupport && (targetBot.type == RobotType.MINER || targetBot.type == RobotType.BUILDER)) break;
            if (rc.canAttack(targetBot.location)) {
                rc.attack(targetBot.location);
                broadcastTarget(targetBot.location);
                return targetBot;
            }
        }
        return null;
    ...
    public RobotInfo attemptAttackOld(boolean attackMiners) throws GameActionException {
        RobotInfo[] nearbyBots = rc.senseNearbyRobots(RobotType.SOLDIER.actionRadiusSquared, rc.getTeam().opponent());
