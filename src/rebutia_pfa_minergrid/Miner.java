...
public class Miner extends Unit {
    ...
    int exploratoryGridSquare = -1;
    MapLocation exploratoryMapLocation;
    ...
    public Miner(RobotController rc) throws GameActionException {
        ...
        exploratoryGridSquare = rc.readSharedArray(CHANNEL.MINERS_BUILT.getValue()) % 16;
        rank = findRankMiner();

        setExploratoryMapLocationByExploratoryGridSquare(exploratoryGridSquare);
    }

    private void setExploratoryMapLocationByExploratoryGridSquare(int exploratoryGridSquare) {
        int gridX = exploratoryGridSquare % 4;
        int gridY = exploratoryGridSquare / 4;
        int gridSquareWidth = rc.getMapWidth() / 4;
        int gridSquareHeight = rc.getMapHeight() / 4;
        int x = gridX * gridSquareWidth + gridSquareWidth / 2;
        int y = gridY * gridSquareHeight + gridSquareHeight / 2;
        exploratoryMapLocation = new MapLocation(x, y);
    }
            ...
            case DEFAULT:
                switch (mode) {
                    ...
                    case FLEEING:
                        ...
                        break;
                    case MOVING_TO_EXPLORATION_ASSIGNMENT:
                        // clear the assignment
                        if (rc.getLocation().distanceSquaredTo(exploratoryMapLocation) < 10) {
    ...
    public boolean isFleeMode_withSideEffects() throws GameActionException {
        ...
        if (potFleeDirection != null || stopFleeingRound <= round_num) {
            ...
            if (potFleeDirection != null)
                fleeDirection = potFleeDirection;
    ...
    public MapLocation findMiningAreaWithBroadcast() throws GameActionException {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            if (data != 0) {
                ...
                int x = (data >> 4) & 15;
                int y = data & 15;
                MapLocation dest = new MapLocation(Math.min(x * 4, rc.getMapWidth() - 1),
                        Math.min(y * 4, rc.getMapHeight() - 1));
                // System.out.println("Recieved miner request: " + x*4 + " " + y*4 + " " +
                // demand);
                int res = minerToLeadRate * demand;
                if (my.distanceSquaredTo(dest) < 300 && res > maxRes && demand > 0) {
                    // within range
                    // TODO: add not fulfilled
                    maxRes = res;
                    bestLocation = dest;
                    channel = i;
                }
        ...
        if (maxRes >= 1 && bestLocation != null) {
            ...
            int x = (data >> 4) & 15;
            int y = data & 15;
            // System.out.println("Taking miner request: " + x*4 + " " + y*4 + " " +
            // demand);
