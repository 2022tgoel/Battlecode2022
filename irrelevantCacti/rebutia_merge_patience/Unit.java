package rebutia_merge_patience;
...
public class Unit {
    ...
    private Direction waiting__bestDirection = null;
    private int waiting__numTurns = 0;
     ...
     * easy replacement)
     * 
     * @param dest is where to go
     **/
    public void moveToLocation(MapLocation dest) throws GameActionException {
        Direction bestDirection_unoccupied = mover.getBestDir(dest);
        Direction bestDirection_overall = mover.getBestDir_withOccupied(dest);
        Direction directionToTravel = null;
        if (!waiting__bestDirection.equals(bestDirection_overall)) {
            waiting__bestDirection = bestDirection_overall;
            waiting__numTurns = 0;
        }
        if (bestDirection_overall.equals(bestDirection_unoccupied)) {
            // The truly best direction is unoccupied
            directionToTravel = bestDirection_overall;
        } else {
            // The truly best direction is occupied
            waiting__bestDirection = bestDirection_overall;
            waiting__numTurns++;

            // Calculate the difference in rubble
            MapLocation t_1 = rc.getLocation().add(bestDirection_unoccupied);
            MapLocation t_2 = rc.getLocation().add(bestDirection_overall);
            int bestRubble_1 = rc.senseRubble(t_1);
            int bestRubble_2 = rc.senseRubble(t_2);
            int maxWaitingTurns = (int) (Math.ceil((bestRubble_1 - bestRubble_2) / 10.0) / 2);

            if (waiting__numTurns > maxWaitingTurns) {
                // We've waited too long, so we'll just move in the best
                // direction
                directionToTravel = bestDirection_unoccupied;
            }
        }
        if (directionToTravel != null && rc.canMove(directionToTravel)) {
            // Every time we move, clear the numTurnsWaitingToMove
            waiting__bestDirection = null;
            waiting__numTurns = 0;
            rc.move(directionToTravel);
        }
    }

    public void moveInDirection(int[] direction) throws GameActionException {
        MapLocation me = rc.getLocation();
        MapLocation dest = new MapLocation(me.x + direction[0], me.y + direction[1]);
        moveToLocation(dest);
    }
